...
function writeAccessTokensToFile(tokensObj) {
    fs.writeFile("access_tokens.json", JSON.stringify(tokensObj), (err) =>
        console.log(err)
    );
}
...
app.post(
    ...
    async (req, res, next) => {
        ...
        let { handle, profilePictureUri, followNFTURI, followModule, bio } =
            req.query;
        ...
        try {
            ...
            let signature = await signMessage(
                setProfileMetadataResponse.data.data
            ...
            let broadcastResponse = await broadcastTransaction(
                ...
                setProfileMetadataResponse.data.data
                    .createSetProfileMetadataTypedData.id,
                signature
            );
...
app.post(
    ...
    async (req, res, next) => {
        ...
        let conditions = profile.attributes.filter(
            (attribute) => attribute.traitType == "nft"
        );
        ...
        if (walletSatisfiesConditions) {
            ...
            let contentURI = await uploadToIpfs(
                "Comment_by_@youmemeworld.lens",
                contentMetadata
            );
            ...
            let signature = await signMessage(
                createPostTypedDataResponse.data.data.createPostTypedData
            );
